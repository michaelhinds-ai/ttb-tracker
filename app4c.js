...
async function sendSalesTest(){
  ...
  const el=$('#s_salesEmail'); const to=((el&&el.value)||'').trim();
  const msg=$('#salesTestMsg');
  if(!to){ if(msg){msg.textContent='Enter at least one email first.';msg.style.color='var(--red)';} return; }
  state.settings.salesEmailTo=to; save('Updated nightly sales email recipients');
  if(msg){ msg.textContent='Sending…'; msg.style.color='var(--muted)'; }
  try{
    const r=await fetch('/api/sales/test',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({to})});
    const d=await r.json().catch(()=>null);
    if(d&&d.ok){ if(msg){ msg.textContent='Sent to '+(d.sentTo||[]).join(', ')+' — check your inbox.'; msg.style.color='var(--green)'; } }
    else { const why=(d&&(d.detail||d.error))||('HTTP '+r.status); if(msg){ msg.textContent='Could not send: '+why; msg.style.color='var(--red)'; } }
  }catch(e){ if(msg){ msg.textContent='Could not send: '+(e&&e.message||e); msg.style.color='var(--red)'; } }
}
...
function refreshAll(){try{runSyncRepair();}catch(e){}updateBadge();fillYearSelects();renderHome();renderTasks();renderLrs();renderLabels();renderCompliance();renderMarketing();renderBarrelsProc();try{renderExpenses();}catch(e){}renderDashboard();renderLedger();initReportControls();renderReport();initKyControls();renderKy();renderBarrels();renderBottling();renderFinished();renderOrders();renderCustomers();renderUsers();try{renderRolePanel();}catch(e){}renderBackups();dataStatus();syncUI();applyPermissions();}
