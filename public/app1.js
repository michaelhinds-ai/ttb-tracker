...
/* ================= State ================= */
function freshState(){return{settings:{name:'',permit:'',freq:'quarterly',year:new Date().getFullYear(),kyExcise:1.92,kyWholesale:11,kyCase:0.05,bottlesPerCase:6},entries:[],ky:{monthly:{},barrel:{}},barrels:[],bottlings:[],finishedGoods:[],customers:[],orders:[],history:[],auth:{enabled:false,users:[]},dailyBackups:[],tibouts:[],tibins:[],tasks:[],skus:[{id:'seed-lrwc-11sib',company:'Louisville Rickhouse Whiskey Co',product:'11 Year Single Barrel Bourbon',size:'750ml',scc6:'',scc12:'10850033780311'}],brandLogos:{},docs:[],assets:[],barrelsProc:[],wages:{},expenses:[],pnlXolaMap:{}};}
...
function applyPermissions(){
  ...
  ['admin','editor','manager','production','reports','viewer'].forEach(r=>b.classList.toggle('role-'+r, role===r));
...
function renderTasks(){
  ...
  if(sel){
    ...
    sel.innerHTML = authOn()
      ? users.map(u=>`<option value="${u.id}">${esc(u.name||'')}${me&&u.id===me?' (me)':''}</option>`).join('')
...
function completeTask(id){
  ...
  const t=state.tasks[i]; state.tasks.splice(i,1);
